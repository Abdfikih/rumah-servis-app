...
module.exports = {
    ...
    getOrderDetails: async (req, res) => {
        ...
        try {
            const order = await Order.findById(orderId)
                ...
                .populate({
                    path: 'shopId',
                    select: 'name location'  // Fetch the name and location of the shop
                })
                .populate({
                    path: 'driverId',
    ...
    getUserOrders: async (req, res) => {
        ...
        try {
            const orders = await Order.find({ userId }).populate('shopId').populate('driverId');
    ...
    getNearbyOrders: async (req, res) => {
        try {
            ...
            }).select('userId deliveryAddress orderItems deliveryFee shopId shopCoords recipientCoords')
                .populate({
                    path: 'userId',
    ...
    getPickedOrders: async (req, res) => {
        ...
        try {
            ...
            }).select('userId deliveryAddress orderItems deliveryFee shopId shopCoords recipientCoords driverId orderStatus orderStatus orderStatus orderStatus orderStatus orderStatus orderStatus orderStatus')
                .populate({
                    path: 'userId',
                    select: 'phone profile' // Replace with actual field names for suid
                })
