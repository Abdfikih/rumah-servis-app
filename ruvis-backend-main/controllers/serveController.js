const serves = require("../models/serve");

module.exports = {
  addserve: async (req, res) => {
    const newserve = new serves(req.body);
  ...
  getServeById: async (req, res) => {
    ...
    console.log(serveId);
    try {
      const serve = await serves.findById(serveId); // populate the shop field if needed
  ...
  getServeByShop: async (req, res) => {
    const shopId = req.params.id;
    console.log("test serve");

    console.log(shopId);

    try {
      const serves = await serves.find({ shop: shopId });
  ...
  deleteserveById: async (req, res) => {
    ...
    try {
      const serve = await serves.findByIdAndDelete(serveId);
  ...
  serveAvailability: async (req, res) => {
    ...
    try {
      ...
      const serve = await serves.findById(serveId);
  ...
  updateserveById: async (req, res) => {
    ...
    try {
      const updatedserve = await serves.findByIdAndUpdate(serveId, req.body, {
        new: true,
        runValidators: true,
      });
  ...
  getAllserves: async (req, res) => {
    try {
      const serve = await serves.find();

      if (!serve || serve.length === 0) {
        return res
          .status(404)
          .json({ status: false, message: "No serve items found" });
      }
  ...
  getRandomservesByCategoryAndCode: async (req, res) => {
    ...
    try {
      let serves = await serves.aggregate([
